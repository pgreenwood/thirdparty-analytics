...
/**
 * Helper class that consumes output of a process.
 * In addition, it filter output of the REG command on Windows to look
 * for InstallPath registry entry which specifies the location of R.
 */
 ...
 * see mainly <code>checkLocalRserve</code> method. It spits out
 * quite some debugging outout of the console, so feel free to modify
 *  it for your application if desired.<p>
 *
 * <i>Important:</i> All applications should shutdown every Rserve that
 * they started! Never leave Rserve running if you started it after your
 * application quits since it may pose a security risk. Inform the user
 * if you started an Rserve instance.
*/
public class Rserve {
	/** shortcut to <code>launchRserve(cmd, "--no-save --slave", "--no-save --slave", false)</code> */
	public static boolean launchRserve(String cmd) { return launchRserve(cmd, "-q --no-save --slave --no-restore","-q --no-save --slave --no-restore",false); }
	/**
	 ...
	 */
	public synchronized static boolean launchRserve(String cmd, String rargs, String rsrvargs, boolean debug) {
		try {
			Process p;
	 ...
	 * start it using the defaults for the platform where it is run on. This method
	 * is meant to be set-and-forget and cover most default setups.
	 *
	 * <p>For special setups you may get more control over R with
...
//			System.out.println("First connect try failed with: " + e.getMessage());
	 ...
	 */
	public synchronized static boolean shutdownRserve() {
		try {
			RConnection c = new RConnection();
			c.shutdown();
		} catch (RserveException x) {
			System.out.println("Rserve Already down: " + x.getMessage());
			return true;
		}
		return true;
	}
